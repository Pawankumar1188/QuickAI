...
//             });
//         }
//         //clipdrop api to generate the image
...
const getAiErrorResponse = (error) => {
    ...
    if (status === 429) {
        return {
            status,
            message: "Gemini rate limit or quota reached. Wait a minute and try again, or check your API key quota and billing.",
        };
    }

    return {
        status,
        message: error?.message || "Something went wrong while contacting the AI service.",
    };
};

const sendAiError = (res, error) => {
    const { status, message } = getAiErrorResponse(error);
    console.error("AI request failed:", status, error?.message);
